"""
Monte Carlo nonlinear policy optimization for the inertia-wheel pendulum.
...
The gradients are averaged across initial-state samples and updated with plain
batch gradient descent:

    theta <- theta - learning_rate * mean_gradient.

No Adam or momentum is used.

Required files/packages:
    - deer.py containing deer_alg, available on the Python path
    - pip install "jax[cpu]" numpy matplotlib pillow
"""
...
def save_gif(
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Inertia-wheel pendulum: MC two-pass DEER + GD")
...
def main() -> None:
    ...
    print("Inertia-wheel pendulum: two-pass DEER + Monte Carlo gradient")
